...
const Results: React.FC = () => {
    ...
    const calcularNecesidadesCaloricas = (): number => {
        let TMB: number = Math.pow(peso, 0.75) * 70; // Tasa Metabólica Basal
    ...
    const calcularRacion = (): string => {
        ...
        const caloriasDiarias: number = calcularNecesidadesCaloricas();
        const caloriasPorGramo: number = 350 / 100; // 350 kcal por cada 100 gr
        const racionGramos: number = caloriasDiarias / caloriasPorGramo;

        return `${racionGramos.toFixed(0)} gr. de pienso al día aproximadamente.`;
    };

    const racionRecomendada: string = calcularRacion();
    ...
    return (
        <>
            ...
            <Flex direction="column" minHeight="100vh">
                {" "}
                <Box bgGradient="linear(to-r, #ffde59, #ffc700)" mt={{ base: "35%", md: "20%", lg: "12%" }} ml="-5vh" w={{ base: "100%", md: "80%", lg: "40%" }} p="2vh" borderRadius="full">
